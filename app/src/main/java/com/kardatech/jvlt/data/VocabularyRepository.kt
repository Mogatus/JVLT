...
class VocabularyRepository(private val vocabularyDao: VocabularyDao) {
    ...
    fun getCategoriesStream(language: String): Flow<List<String>> = vocabularyDao.getCategories(language)
    fun getTriesStatsStream(language: String): Flow<List<TriesStat>> = vocabularyDao.getTriesStatistics(language)
    ...
    suspend fun getRandomItemFiltered(language: String, category: String?, stages: List<Int>): VocabularyItem? = 
        vocabularyDao.getRandomItemFiltered(language, category, stages)
